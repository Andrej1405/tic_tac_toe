...
export default class OpponentComputer extends TicTacToe {
    ...
    computerLogic = () => {
        ...
        const zeroField = this.currentGameMatrix[constants.FIELD_NUMBER_ZERO]
        const secondField = this.currentGameMatrix[constants.FIELD_NUMBER_TWO]
        const sightField = this.currentGameMatrix[constants.FIELD_NUMBER_SIX]
        const eightField = this.currentGameMatrix[constants.FIELD_NUMBER_EIGHTEEN]

        switch (true) {
            case zeroField !== constants.VALUE_CROSS && zeroField !== constants.VALUE_CIRCLE:
                this.fields[constants.FIELD_NUMBER_ZERO].dispatchEvent(this.event)
                isCompletedTurn = true
                return
            case secondField !== constants.VALUE_CROSS && secondField !== constants.VALUE_CIRCLE:
                this.fields[constants.FIELD_NUMBER_TWO].dispatchEvent(this.event)
                isCompletedTurn = true
                return
            case sightField !== constants.VALUE_CROSS && sightField !== constants.VALUE_CIRCLE:
                this.fields[constants.FIELD_NUMBER_SIX].dispatchEvent(this.event)
                isCompletedTurn = true
                return
            case eightField !== constants.VALUE_CROSS && eightField !== constants.VALUE_CIRCLE:
                this.fields[constants.FIELD_NUMBER_EIGHTEEN].dispatchEvent(this.event)
                isCompletedTurn = true
                return
        }
